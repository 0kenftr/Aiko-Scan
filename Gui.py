...
Serverdetect = config['config']['Serverdetect']
Serverport = config['config']['Serverport']
Bancode = config['config']['Bancode']
Player_1 = config['config']['Playername_1']
Player_2 = config['config']['Playername_2']

if Serverdetect == "":
    Serverdetect = "Serverdetect : False"
if Serverport == "":
    Serverport = "Serverport : 00000"
if Bancode == "":
    Bancode = "Bancode = A5manw27VC"
if Player_1 == "":
    Player_1 = "Aiko - Studio"
if Player_2 == "":
    Player_2 = "Cheat - jar"
...
def load_file():
    ...
    if logpath:
        ...
        #config hightlight
        highlight_red(Serverdetect)
        highlight_red(Serverport)
        highlight_red(Bancode)
        highlight_red(Player_1)
        highlight_red(Player_2)
        #movement modules
...
Checkbutton = PhotoImage(file='library/main/image/gui/Comp 10.png')
importbutton = PhotoImage(file='library/main/image/gui/importbutton.png')

c = Canvas(win, bg="gray16", height=650, width=1180)
c.pack()
frames = [PhotoImage(file=f"library/main/image/frame/Comp {i}.png") for i in range(15900, 15959)]
